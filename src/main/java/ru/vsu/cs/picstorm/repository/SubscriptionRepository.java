...
public interface SubscriptionRepository extends JpaRepository<Subscription, Long> {
    @Query(value = "SELECT s FROM Subscription s WHERE s.subscriber = ?1 ORDER BY s.target.nickname")
    Page<Subscription> findPageBySubscriber(User subscriber, Pageable pageable);
    @Query(value = "SELECT s FROM Subscription s WHERE s.target = ?1 ORDER BY s.subscriber.nickname")
    Page<Subscription> findAllByTarget(User target, Pageable pageable);
